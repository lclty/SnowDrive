...
// ─── Logout (Demo - just reload to reset state) ────────────────────

document.addEventListener('DOMContentLoaded', () => {
    const logoutBtn = document.getElementById('logout-btn');
    if (logoutBtn) {
        logoutBtn.addEventListener('click', async () => {
            // Clear demo cookies to reset state
            document.cookie = 'snowdrive_vfs=; max-age=0; path=/';
            document.cookie = 'snowdrive_settings=; max-age=0; path=/';
            try {
